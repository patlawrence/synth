module.exports = class CooldownsManager {
    isOnCooldown(message, command) {
        ...
        if(users.has(authorID)) return true;
    ...
    putOnCooldown(message, command) {
		...
		if(!guilds.has(guildID)) guilds.set(guildID, new Collection()); // add guild to cooldowns
			
		const guild = guilds.get(guildID);

        if(!guild.has(command.name)) guild.set(command.name, new Collection()); // add command to guild specific cooldowns
        ...
        setTimeout(() => { // after a specified amount of time
			...
			if(!usersOnCooldown.size) guild.delete(command.name); // delete command from guild cooldowns
				
            if(!guild.size) guilds.delete(guildID); // delete guild from cooldowns
